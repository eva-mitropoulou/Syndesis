...
**Methods.** Syndesis combines five-state ensemble docking, GNINA rescoring, and ProLIF fingerprints. For each target, the primary interaction prior was the union of residue-by-interaction-type bits observed across five native complexes. The prespecified ranking rule selected the maximum, over receptor states, of CNNscore multiplied by one plus same-pose prior recall. EGFR and CDK2 DUD-E benchmarks were evaluated with paired class-stratified bootstraps, three permutation nulls, native-ligand-overlap exclusions, leave-one-receptor-out analyses, prior-definition and formula sensitivity analyses, and replicated 20 ns molecular dynamics (MD) of selected complexes.

**Results.** Strict graph-preserving fingerprint recomputation on EGFR raised EF1% from 11.79 for GNINA to 16.21 for pose-coupled weighting. Excluding the native complex whose ligand exactly overlaps a DUD-E active retained an EF1% of 16.40, and all five native-complex and receptor-state leave-one-out analyses preserved the direction of the effect. CDK2 showed a favorable EF1% difference of 2.11, although its paired 95% interval crossed zero. Twenty-one production trajectories separated a deliberately mis-docked control from four complexes that passed the majority-replicate stability rule. The prospective application produced structurally annotated hypotheses and no activity labels.
...
Protein-ligand interaction fingerprints reduce a complex to explicit residue-by-interaction descriptors. Structural interaction fingerprints, SPLIF-like encodings, kinase interaction profiles, and modern implementations such as ProLIF have been used to compare poses, analyze binding modes, and rescore docking results [@deng2004; @chuaqui2005; @marcou2007; @daSilva2012; @bouysset2021]. These representations offer interpretability, but their use creates additional choices: which native complexes define the target, whether the reference is a union or a conserved core, how receptor numbering is normalized, and whether the interaction term is evaluated on the same pose as the docking score.
...
Protein structures were obtained from the Protein Data Bank [@berman2000]. The EGFR ensemble comprised 1M17/AQ4, 1XKK/FMM, 4HJO/AQ4, 5CAV/4ZQ, and 6DUK/JBJ, spanning active-like and inactive-like kinase conformations. The CDK2 transfer ensemble comprised 1QMZ/ATP, 1FIN/ATP, 2A4L/RRC, 1AQ1/STU, and 1PXN/CK6; 1H00 was reserved as a structural holdout. Chain identifiers, mutations, ligand identifiers, quality decisions, pocket-state annotations, and docking-box coordinates are retained in the repository's receptor and complex tables.

Receptors were aligned through conserved ATP-pocket residues, and residue identifiers used by interaction fingerprints were normalized through target-specific mapping tables. Missing residues contributed no bit for the affected complex; mutations retained the normalized positional identifier while preserving the observed residue name in the source table. Docking boxes were receptor specific and derived from the aligned native ATP-site ligand. The 6DUK structure was retained solely as an ATP-pocket conformation; its crystallographic ligand is allosteric, motivating a prespecified receptor-exclusion analysis.
...
|---|---|---:|---:|
| EGFR | 1M17, 1XKK, 4HJO, 5CAV, 6DUK | 35,552 (542 actives; 35,010 decoys) | 5 |
| CDK2 | 1QMZ, 1FIN, 2A4L, 1AQ1, 1PXN | 28,296 (474 actives; 27,822 decoys) | 5 |

## Ligand preparation, docking, and neural rescoring

DUD-E SMILES were parsed with their original identifiers [@mysinger2012]. RDKit generated one hydrogen-complete three-dimensional structure per input using ETKDGv3 and seed `0xF00D`; MMFF optimization was attempted for up to 1,000 iterations [@landrum2013; @wang2004]. Open Babel 3.1.0 converted SDF structures to PDBQT and assigned Gasteiger charges [@oboyle2011]. Preparation failures were recorded rather than replaced by alternative chemistry.

Each ligand was docked independently against all five receptor states with Uni-Dock 1.2.0 in `balance` search mode, nine output modes, seed 807, and GPU-batched execution [@yu2023unidock]. Receptor-specific centers and dimensions are provided in the machine-readable docking-box tables. The top Uni-Dock pose for each ligand-receptor pair was rescored with GNINA 1.3.3 using the distributed default ensemble in score-only/rescore mode [@mcnutt2021; @mcnutt2025]. Ligand-level GNINA ranking was the maximum CNNscore across the five receptor-specific poses.
...
Docking PDBQT files do not preserve a complete chemical graph. For every scored pose, docked heavy-atom coordinates were therefore mapped strictly onto the corresponding prepared SDF graph before fingerprinting. Mapping required matching heavy-atom element sequences and counts; failures blocked analysis. Explicit hydrogens used only to define input stereochemistry were removed from the graph while their implicit hydrogen state was retained. All 177,760 EGFR ligand-receptor poses passed this graph-preserving reconstruction.
...
which contained 76 bits for EGFR and 47 for CDK2. Same-pose prior recall was
...
with $\lambda=1$ fixed for the primary comparison after method development. CNNscore and $R_{i,r}$ within each product belong to the same pose. This differs from late fusion, in which the best neural score and interaction score may come from different receptor states. Formula sensitivity covered $\lambda\in\{0,0.25,0.5,1,1.5,2,3\}$, additive combination, rank fusion, and recall, Jaccard, precision, and F1 interaction terms.

## Statistical analysis and controls

The primary endpoint was EF1%; ROC-AUC, EF5%, and BEDROC with $\alpha=80.5$ were secondary endpoints [@truchon2007]. The top set contained `round(N x fraction)` molecules, with a minimum of one, and descending ties were resolved by stable input order. Uncertainty intervals used 2,000 class-stratified bootstrap resamples with seed 807. Both methods were evaluated on the same resampled active and decoy indices, and paired differences were retained within each replicate.

Three 1,000-permutation nulls distinguished different alternative explanations. The all-ligand null reassigned each ligand's five-receptor recall vector across the complete benchmark and therefore removed label-associated recall structure. The property-matched null shuffled those vectors within heavy-atom-count deciles without conditioning on activity. The class-conditional assignment null shuffled within active and decoy classes separately; it preserves class-level recall distributions and tests only whether the correct molecule-specific assignment adds information. Empirical one-sided $p$ values were calculated as $(b+1)/(1000+1)$, where $b$ is the number of null EF1% values at least as large as observed.

Robustness analyses removed each docking receptor in turn and each native complex in turn. Native-ligand leakage was audited by canonical identity, Bemis-Murcko scaffold, and ECFP4 similarity [@bemis1996; @rogers2010]. The primary prior was also rebuilt after excluding all exact native/DUD-E overlaps. Spearman correlations quantified association between union recall and heavy-atom count, molecular weight, and total pose-contact count.
...
Seven complexes (three known controls, three deterministic analogs, and one deliberately mis-docked negative control) underwent three independent 20 ns production trajectories each. Protein parameters used AMBER ff19SB, ligand parameters used GAFF2 with AM1-BCC charges generated by AmberTools 24.8 and ACPYPE 2023.10.27, and solvent used the OPC3 model [@case2023; @jakalian2000; @jakalian2002; @izadi2014; @izadi2016]. GAFF2 parameterization warnings and net charges were retained per ligand; these open parameters are reproducible estimates, not a claim of force-field perfection.
...
GNINA was a strong baseline, with ROC-AUC 0.766, EF1% 11.79, EF5% 6.90, and BEDROC 0.209. Pose-coupled native-union weighting increased these values to 0.770, 16.21, 7.64, and 0.279, respectively. In practical terms, GNINA retrieved 64 actives among the first 356 molecules, whereas the coupled arm retrieved 88, an increase of 24 known actives at the 1% cutoff.
...
| **Coupled score** | **0.770 (0.747–0.794)** | **16.21 (13.27–19.16)** | **7.64 (6.83–8.45)** | **0.279 (0.241–0.316)** |
...
![Docking, GNINA, and pose-coupled interaction weighting on EGFR and CDK2. Error bars show 2,000-bootstrap 95% intervals.](figures/figure2_enrichment.png){#fig-enrichment width=100% fig-alt="Bar charts for ROC-AUC, EF1%, and BEDROC across EGFR and CDK2 ranking arms."}

All paired contrasts favored the coupled arm. The EF1% gain was 4.42 (95% CI 2.40–6.63), the EF5% gain was 0.74 (0.33–1.11), the ROC-AUC gain was 0.0048 (0.0034–0.0061), and the BEDROC gain was 0.0707 (0.0536–0.0880). Every bootstrap difference was positive for each metric.

The three permutation nulls answered different questions. An unrestricted all-ligand shuffle gave mean EF1% 11.46 (95% permutation range 9.95–12.72), corresponding to an observed-minus-null increment of 4.76 and empirical $p=0.0010$. Heavy-atom-decile matching gave mean 12.33 (11.05–13.64), increment 3.88, and $p=0.0010$. The class-conditional assignment null was more stringent because it preserved active-decoy recall differences: its mean was 14.14 (12.71–15.48), leaving an increment of 2.08 and $p=0.0030$. The latter is therefore a class-conditional molecule-assignment test, not a general random-score null.

![Permutation distributions for unrestricted, heavy-atom-matched, and class-conditional interaction assignments.](figures/figure4_permutation_nulls.png){#fig-permutation width=100% fig-alt="Permutation distributions of EF1 percent for EGFR and CDK2, with observed coupled-score enrichment marked."}

No single docking receptor explained the effect. Across all five leave-one-receptor-out analyses, EF1% gains ranged from 2.95 to 4.24 and all paired 95% intervals excluded zero. Excluding 6DUK yielded GNINA EF1% 11.98 and coupled EF1% 16.03, a difference of 4.05 (2.21–6.26).
...
Native-ligand overlap did not account for the EGFR result. FMM from 1XKK was identical to one DUD-E active (maximum ECFP4 Tanimoto 1.0 and a matching Bemis-Murcko scaffold); the other native ligands had maximum active-set similarities of 0.618 or less and no exact identity. Rebuilding the prior without 1XKK/FMM gave EF1% 16.40. Across all five leave-one-native-complex-out unions, EF1% ranged from 16.03 to 16.95. Alternative definitions gave EF1% 16.21 for the 60% conserved core, 16.58 for frequency weighting, 16.03 for union Jaccard, and 16.95 for receptor-specific recall.

Union recall was correlated weakly with heavy-atom count ($\rho=0.168$) and molecular weight ($\rho=0.174$) across all receptor-specific poses, but strongly with total detected contacts ($\rho=0.736$). For the selected coupled pose per ligand, the corresponding correlations were 0.198, 0.206, and 0.757. The heavy-atom-matched null addresses part of this size structure; the contact-count correlation remains a reason to report alternative normalized interaction terms rather than interpret union recall as a size-free quantity.

Formula sensitivity did not identify $\lambda=1$ as a uniquely favorable setting. Within the multiplicative recall family, EF1% rose from 13.27 at $\lambda=0.25$ to 17.87 at $\lambda=3$; the prespecified $\lambda=1$ result lay within this monotonic range. Additive and rank-fusion variants also improved early enrichment for several interaction terms. These analyses support complementarity but do not establish a universal functional form or weight.
...
This is an important boundary rather than a negative result to be hidden. All three CDK2 permutation tests placed the observed EF1% above their null means (empirical $p=0.0010$, 0.0140, and 0.0220 for unrestricted, heavy-atom-matched, and class-conditional assignment), but receptor exclusions were heterogeneous: excluding 1AQ1 gave a difference of -0.21, whereas excluding 1FIN gave +3.16. Removing both native ligands with exact DUD-E-active identities reduced EF1% from 13.08 to 12.45. The evidence therefore does not establish a receptor-independent or target-agnostic gain; it supports evaluating the incremental contribution separately for each target and ensemble.
...
The archived ZINC tranche collection contained 103,570 rows and 103,558 unique identifiers. The prespecified property filter retained 25,870 molecules, and the two-per-scaffold cap retained 21,599. The first 2,000 records in deterministic source order were selected, of which 1,993 yielded complete five-receptor scores and strict fingerprints. The selected-pose thresholds were CNNscore 0.7065 and native-union recall 0.25; 137 compounds passed both. The top 50 gate-passing compounds represented 50 distinct Bemis-Murcko scaffolds.

This endpoint is intentionally modest. The output is a tranche-derived set with recorded structural rationale, not a set of predicted actives or a verified availability catalogue. The highest coupled-scoring gate-passing candidate, ZINC11705051, had CNNscore 0.945, interaction recall 0.316, and coupled score 1.244. These values prioritize a pose for inspection and experimental testing, but do not establish affinity, selectivity, or current vendor availability.
...
The central result is specific. On the EGFR benchmark, native-union recovery improved early enrichment when it was coupled to the neural score of the same pose. The paired bootstrap, three permutation designs, five docking-receptor exclusions, and five native-complex exclusions address sampling variation, generic score combination, property structure, label-associated recall, receptor dependence, and native-ligand overlap. Together they support an EGFR ranking improvement, not a general claim that interaction fingerprints solve docking.
...
Syndesis adds a target-native interaction check to ensemble docking and GNINA rescoring without discarding either source of evidence. On EGFR, the pose-coupled rule improved EF1%, EF5%, ROC-AUC, and BEDROC relative to GNINA; the effect survived unrestricted, property-matched, and class-conditional permutation nulls, every receptor exclusion, and removal of the exact native/DUD-E ligand overlap. The workflow is best viewed as a framework for auditable structural prioritization: it records why a candidate was retained, why another was rejected, and what remains to be established experimentally.
...
Source code, workflow configurations, tests, figures, the rendered manuscript, and compact machine-readable result tables are available at [https://github.com/eva-mitropoulou/Syndesis](https://github.com/eva-mitropoulou/Syndesis). Raw structures and benchmark molecules originate from the PDB, DUD-E, and ZINC22 and remain subject to their source terms. Large docking intermediates and MD trajectories are not committed to Git; checksums, source manifests, deterministic rebuild scripts, and analysis-ready summaries are provided instead. No separate supplementary document accompanies this manuscript.

The manuscript is available as a ChemRxiv preprint (2026; DOI pending).
