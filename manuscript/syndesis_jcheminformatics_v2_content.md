...
For the EGFR ensemble, the documented author chain was exported with Bio.PDB and restricted to standard amino-acid residues. Co-crystallized ligands, waters, ions, cofactors, and other hetero-residues were removed from the docking receptor; pocket waters were exported separately for audit but were not retained. Ligand alternate locations were flagged during structural quality assessment; no alternate-conformer ensemble was modeled. Active-site residue and atom coverage was screened before receptor selection, and missing residues were not modeled into docking receptors. Open Babel 3.1.0 converted the cleaned chain to receptor PDBQT using `-xr -p 7.4 --partialcharge gasteiger`; this added hydrogens, applied pH 7.4 protonation, and assigned Gasteiger receptor charges. Histidine states therefore follow Open Babel's pH-dependent assignment rather than a manually selected histidine microstate. For each docked-pose interaction calculation, the ProLIF protein was regenerated from that exact PDBQT receptor, not independently repaired. A five-receptor audit found identical docking and ProLIF heavy-atom sets, with zero ProLIF-only atoms for every receptor; therefore no interaction was calculated against a protein heavy atom absent during docking. PDBFixer was used only for complete native-complex inputs without a docking PDBQT counterpart and fails closed rather than repairing missing protein heavy atoms.

The CDK2 transfer preparation used the dedicated chain-A extractor. In 1QMZ, phosphothreonine TPO160 is encoded as `HETATM`; the extractor retained only `ATOM` records, so this receptor was represented as unphosphorylated and no phosphothreonine parameters were claimed. The existing 1QMZ-excluded sensitivity is consequently the relevant CDK2 robustness check, rather than evidence for a phosphorylated-CDK2 model.
...
The structural audit used scaffold-holdout pose ranking and a default-deny feature policy: variables that directly encode native agreement or downstream design decisions were excluded from model training. The selected supporting model was a LightGBM `rank_xendcg` ranker with 200 estimators. Ranking groups were docking tasks; the deterministic scaffold split used 60%/20%/20% train/validation/test partitions and seed 604. Its 227 features combined docking and GNINA scores, pose rank and sanity variables, receptor-state descriptors, ligand descriptors, binding-mode-cluster variables, and interaction bits that passed the training-frequency filter. Hyperparameters and model family were chosen by validation NDCG@1, while the reported NDCG@3 was evaluated on the held-out scaffold split.

For an analog and its parent, the pre-MD candidate score was $C=0.55q+0.25r+0.20s$, where $q$ is pose confidence, $r$ is selected-pose key-interaction recall, and $s$ is GNINA CNNscore. The strict gate required hard chemical-scope validity, binding-mode preservation in the same receptor/pose comparison, $q\geq0.30$, $\Delta C\geq-0.02$, $\Delta$ ligand efficiency $\geq-0.10$, and no score-hacking flag (higher CNNscore without binding-mode preservation). The held-out diagnostic defined interaction loss from a separately retained interaction-fidelity quantity and was reported at losses exceeding 0.01, 0.02, and 0.05 rather than used to calibrate activity.
...
Trajectories were made whole with `gmx trjconv -pbc mol -center`, centered on protein, globally fit on the protein backbone, and stripped of water. Each frame was then locally aligned on the backbone atoms of residues within 6 Å of the reference ligand; ligand RMSD used all ligand heavy atoms relative to the first corrected production frame. Pocket retention was the fraction of frames in which at least one ligand-heavy-atom to initial-pocket-heavy-atom distance was no greater than 6 Å. Interaction occupancy was the fraction of stored production frames with a minimum heavy-atom distance at or below the interaction-type cutoff (3.6 Å for hydrogen-bond classes and 4.5 Å for hydrophobic or ionic classes); hinge occupancy was the mean over hinge-labelled key interactions. Binding-mode persistence was the mean occupancy over the frequency-filtered key-interaction set. MDAnalysis 2.10.0 and GROMACS trajectory tools generated these quantities using periodic minimum-image distances.
...
The CDK2 analysis prevents overgeneralization. Its EF1% direction was favorable, but the paired uncertainty interval crossed zero and receptor exclusions were heterogeneous. Excluding the unphosphorylated representation of 1QMZ increased the EF1% difference to 2.74 (95% CI 0.00 to 5.06), but this is a sensitivity result, not validation of phosphorylated CDK2 chemistry. The correct interpretation is not that the EGFR effect was reproduced without qualification. It is that the workflow exposes where a target-specific interaction prior may be useful and where its evidence remains incomplete.
...
The downstream results are similarly bounded. The deterministic analog audit is a small-sample diagnostic. The MD calculations test short-term pose persistence and interaction retention, not binding affinity, kinetics, or residence time. The prospective compounds are screening hypotheses. Parameterization and MD use an open AMBER/GAFF2 workflow, which is documented and reproducible but not a claim of force-field perfection. Finally, DUD-E is external to Syndesis method development but is not necessarily independent of the training structures or chemistry seen by GNINA; the paired comparison supports the incremental contribution beyond GNINA on this benchmark, not an absolute estimate of GNINA generalization.
