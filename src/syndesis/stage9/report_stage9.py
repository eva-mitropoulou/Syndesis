...
def write_stage9_report(paths: dict[str, Path]) -> Path:
    ...
    target = paths["reports"] / "09_deterministic_analog_audit.html"
    target.write_text(
        f"""<!doctype html>
<html><head><meta charset=\"utf-8\"><title>Stage 9 Deterministic Analog Audit</title>
...
<body>
<h1>Stage 9 - Deterministic analog generation and structural audit</h1>
<p><strong>Non-claim:</strong> all rows are computational proposals only. No experimental EGFR activity is claimed.</p>
<p>Versioned RDKit transformations generate analogs; Stage 8 docking/GNINA/ProLIF and Stage 6 pose confidence determine survival.</p>
